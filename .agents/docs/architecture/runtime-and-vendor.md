...
| `__ODC_RUNTIME_BUILD__` in `packages/runtime-bundler/src/bundler.ts` ↔ the two react wrappers reading it                                                                                    | the served react stops deferring to the host's, and hooks in a block throw     |
