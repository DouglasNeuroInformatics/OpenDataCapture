...
export const DisclaimerProvider: React.FC<{ children: React.ReactElement }> = ({ children }) => {
  ...
  return (
    <React.Fragment>
      {children}
      <Dialog data-testid="disclaimer-dialog" open={!isDisclaimerAccepted}>
        <Dialog.Content data-testid="disclaimer-dialog-content" onOpenAutoFocus={(event) => event.preventDefault()}>
          <Dialog.Header>
            <Dialog.Title>
              {t({
                en: 'Disclaimer',
            ...
            <Dialog.Description>
              {t({
                ...
                es: 'Esta plataforma no es una historia clínica electrónica. Nuestras condiciones de servicio prohíben usarla como mecanismo principal para almacenar datos clínicos.',
          ...
          <Dialog.Footer>
            <Button data-testid="accept-disclaimer" type="button" onClick={() => setIsDisclaimerAccepted(true)}>
              {t({ en: 'Accept', es: 'Aceptar', fr: 'Accepter' })}
            </Button>
            <Button data-testid="decline-disclaimer" type="button" variant="outline" onClick={logout}>
