...
By default, an admin user has full access (i.e., create, read, update, and delete) to all resources. Only an admin can create, modify or delete users, or change their permissions; no additional permission extends this to anyone else. For security reasons, it is recommended to create only one admin, and to grant group managers additional permissions when required.

#### Group Manager

Group managers can manage their own groups and the remote assignments in them, and read the data collected for their groups (e.g., subjects, sessions and instrument records). They can see the users in their groups, but cannot create, modify or delete them.
