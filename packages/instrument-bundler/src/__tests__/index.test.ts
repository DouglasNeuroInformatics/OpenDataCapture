...
describe('InstrumentBundler', () => {
  ...
  describe('generateBundle', () => {
    it('should successfully transpile the click task', async () => {
      await expect(bundler.generateBundle(interactiveInstrument.source)).resolves.toBeTypeOf('string');
    });
    it('should successfully transpile the happiness questionnaire', async () => {
      await expect(bundler.generateBundle(unilingualFormInstrument.source)).resolves.toBeTypeOf('string');
    });
