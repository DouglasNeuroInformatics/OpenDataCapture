...
// @ts-expect-error - this is implemented here, but only used once, where I just replaced it, so we will not type it since I don't want this used if possible
Number.prototype.isPositive = function () {
  return this.valueOf() > 0;
};

const isNatural = (arg: number) => arg.valueOf() % 1 == 0 && arg.valueOf() > 0;

// @ts-expect-error - this is implemented here, but only used once, where I just replaced it, so we will not type it since I don't want this used if possible
Number.prototype.isNatural = function () {
  return isNatural(this as number);
};

// @ts-expect-error - this is implemented here, but not used, so we will not type it since I don't want this used if possible
...
Array.prototype.zip = function (a: any[]) {
  let r = [];
...
Array.prototype.shuffle = function () {
  ...
  for (
    ...
    i;
    j = parseInt((Math.random() * i) as any as string), x = this[--i], this[i] = this[j], this[j] = x
  ) {
    /* empty */
  }
  return this;
...
// By default, n=2
export function coinFlip(n: number) {
  n = n || 2;
...
export function polygon(n: any, r: any, offsetX: any, offsetY: any) {
...
export function cartesianProduct(a: any, b: any): any {
...
export function vector(a: any, b: any, c: any, d: any) {
  return [a - c, b - d];
}

export function crossProduct(u: any, v: any) {
  let a = u.slice(0);
  let b = v.slice(0);
...
export function magnitude(u: any) {
  let sum = 0;
  let i = u.length;
  let t;
  while (i--) {
...
export function dotProduct(u: any[], v: any[]) {
  let sum = 0;
...
export function maximumDeviation(positions: any[]) {
...
export function areaUnderCurve(positions: any[]) {
  let first = positions[0];
  let last = positions[positions.length - 1];
  let base = vector(first.x, first.y, last.x, last.y);
  let baseLength = magnitude(base);
...
export const zen: Zen = {
  ...
  params: location.href.split('?')[1]
    ? location.href
        .split('?')[1]
        .split('&')
        .reduce(function (cumulative: any, current) {
...
export function $$$(id: string) {
  ...
  return zen.elements[id] as ZenElement | null;
}

// Show slide with id and hide the rest
// Optimizations - it skips DOM elements that already have the
// right visibility value set
export function showSlide(id: string) {
  if (!zen.slides) {
    zen.slides = document.getElementsByClassName('slide') as HTMLCollectionOf<HTMLElement>;
  }
  let change_to;
...
export function degreesToCentimeters(degrees: number, viewingDistance: number) {
...
// Assumes a default viewing distance of 2 feet
export function centimetersToDegrees(centimeters: number, viewingDistance: number) {
 ...
 * returns: none
 */
export function fixOrientation(orientation: 'landscape' | 'portrait') {
  ...
  if (!orientationValues.contains(window.orientation)) {
    alert(message);
  }
 ...
 * returns: none
 */
export function disableTouch(objs: any) {
 ...
 * returns: false
 */
export function getTouchInput(objs: any, fun: any, state: any, duration: any) {
  ...
  let eventListener = function (e: any) {
    e = e || window.event;
    let resp;
  ...
  if (objs === 'document') {
    ...
    for (let i = 0; i < objs.length; i++) {
      objs[i].ontouchstart = function (e: any) {
        if (!objTouched) {
          // @ts-expect-error - legacy
      ...
      objs[i].ontouchend = function (e: any) {
 ...
 * returns: false
 */
export function getKeyboardInput(acceptedKeys: any, fun: any, state: any, duration: any) {
 ...
 */
export function keyValue(code: any) {
...
export function clearChain(a: any[]) {
 ...
 * returns: none
 */
export function preload(images: any, callback: AnyFunction) {
  let self: any = preload;
  ...
  for (let i = 0; i < currentImages.length; i++) {
    ...
    image.onload = function () {
      $$$('_preload_indicator')!.innerHTML = ++self.numLoaded as any;
...
export function generateForm(survey: any, node: any, action: any, method: any, buttonText: any) {
  let self: any = generateForm;

  function tag(kind: any, options: any) {
    let str = '<' + kind + ' ';
    let optStr = [];
  ...
  for (let a = 0, b; (b = survey[a]); a++) {
    ...
    switch (b.type) {
      ...
      case 'checkbox':
      case 'radio':
        b.options.map(function (o: any, i: any) {
          let id = b.name + '[' + i + ']';
          str += tag('span', {
  ...
  $$$(formId)!.validate = function () {
    ...
    survey.map(function (item: any) {
      ...
      if (!item.optional /*&& !item.text*/) {
        ...
        if (!errorEl) {
          ...
          insertAfter($$$('zen_' + id + '_input') as any, errorEl);
        }

        let notBlank = function (val: any) {
          return !(val === '');
        };
    ...
    if (!errorTot) {
      ...
      insertAfter($$$('zen_submit') as any, errorTot);
    }
